...
class Corpus:
    def __init__(self, vocab, word_ids, word_cnt, child_parent, story_contributors, n_topic):
        ...
        self.P = self.Z / np.sum(self.Z, axis = 1)[:, np.newaxis]
...
class HBTP:
    """
    The Discrete Infinite Logistic Normal Distribution (HBTP),
    ...
    def fit(self, corpus, max_iter=100):
        ...
        for iter in range(max_iter):
            ...
            if iter > 3:
                self.lbs.append(lb)
                if iter > 5:
                    if (abs(self.lbs[-1] - self.lbs[-2]) / abs(self.lbs[-2])) < 1e-10:
                        break
    ...
    def update_Z(self, corpus):
        ...
        corpus.Z = corpus.A / corpus.B
        corpus.P = corpus.Z / np.sum(corpus.Z, axis = 1)[:, np.newaxis]
